    ...
    if (validateToken(token)) {
        ...
        if (scoreBoards[uuid] && scoreBoards[uuid].length) {
            scoreBoards[uuid].forEach(s => {
                s.emit(action, {ts: +new Date(), ...(msg || {})}, (response) => {
...
io.on('connection', (socket) => {
    ...
    socket.on('update', ({token, uuid, ...msg}, callback) => forward('update', uuid, token, callback, msg));
    socket.on('ping', ({token, uuid}, callback) => forward('ping', uuid, token, callback));

    socket.on('disconnect', () => {
        console.log('client disconnected');
        uuids.forEach(it => { scoreBoards[it] = (scoreBoards[it] || []).filter(x => x !== socket) })
    });
});
